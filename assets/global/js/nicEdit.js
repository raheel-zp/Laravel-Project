...
var nicLinkOptions = {
    buttons: {
        'link': { name: 'Maskuj Link', type: 'nicLinkButton', tags: ['A'] },
        'unlink': { name: 'Usuń maskowanie linku', command: 'unlink', noActive: true }
    }
};
...
var nicLinkButton = nicEditorAdvancedButton.extend({
    addPane: function() {
        ...
        this.addForm({ "": { type: "title", txt: "Maskuj/Edytuj Link" }, href: { type: "text", txt: "URL", value: "https://", style: { width: "150px" } }, title: { type: "text", txt: "Title" }, target: { type: "select", txt: "Otwórz", options: { "": "w tym oknie", _blank: "w nowym oknie" }, style: { width: "100px" } } }, this.ln)
    },
    submit: function(C) {
        var A = this.inputs.href.value;
        if (A == "https://" || A == "") { alert("You must enter a URL to Create a Link"); return false }
        this.removePane();
        if (!this.ln) {
...
var nicImageButton = nicEditorAdvancedButton.extend({
    addPane: function() {
        ...
        this.addForm({ "": { type: "title", txt: "Add/Edit Image" }, src: { type: "text", txt: "URL", value: "https://", style: { width: "150px" } }, alt: { type: "text", txt: "Alt Text", style: { width: "100px" } }, align: { type: "select", txt: "Align", options: { none: "Default", left: "Left", right: "Right" } } }, this.im)
    },
    submit: function(B) {
        var C = this.inputs.src.value;
        if (C == "" || C == "https://") { alert("You must enter a Image URL to insert"); return false }
        this.removePane();
        if (!this.im) {
