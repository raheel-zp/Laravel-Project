...
<div class="container">
            ...
            <div class="row">
                <div class="col-lg-12">
                    @foreach($jobProves as $job)
                    <div class="job__details__widget">

                        <div class="job__header me-3">
                            <h5 class="job__header-title">
                                <a href="{{ route('job.details', [$job->job->id, slug($job->job->title)]) }}">
                                    {{ __($job->job->title) }}
                                </a>
                            </h5>
                            <p class="job-post-date">
                                {{ showDateTime($job->job->created_at, 'd-m-Y H:i:s') }}
                            </p>
                        ...
                        <div class="icon">
                            <img src="{{ getImage(getFilePath('jobPoster') . '/' . @$job->job->attachment, getFileSize('jobPoster')) }}" alt="@lang('freelancer')" class="img-fluid">
                        </div>
                        <div class="job__details__widget">
                ...
                @if ($jobProves->hasPages($jobProves))
                    {{ paginateLinks($jobProves) }}
                @endif
