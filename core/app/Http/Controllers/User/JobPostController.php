...
class JobPostController extends Controller
{
    ...
    public function approve($id)
    {
        ...
        $activeContest   = Contest::where('contest_state', 1)->get()->toArray();
        if(!empty($activeContest)){
            foreach($activeContest as $contest){
                $activeContestId = $contest["id"];
                $job_added_count = $contest["friend_task_added"];
                $job_completed_count = $contest["friend_task_completed"];
            }
        }
        ...
        if ($ref) {
            $freelancerReferrer = $referrer = $ref->referrer_id;
        }
        ...
        if ($ref) {
            $jobPosterReferrer = $ref->referrer_id;
        }

        $freelancerJob = JobProve::where('user_id', $freelancer->id)->where('status', 1)->whereBetween('updated_at',
        [
            Carbon::now()->startOfMonth(),
            Carbon::now()->endOfMonth()
        ])->get()->toArray();
        $freelancerJobCount = count($freelancerJob);

        $jobPostersJob = JobProve::where('job_post_id', $job_id)->where('status', 1)->whereBetween('updated_at',
        [
            Carbon::now()->startOfMonth(),
            Carbon::now()->endOfMonth()
        ])->get()->toArray();
        $jobPostersJobCount = count($jobPostersJob);


        if(!empty($freelancerReferrer) && $freelancerJobCount == $job_completed_count){
            // add 1 contest point for the Freelance Referrer if already exist in contest details table otherwise add new record with points_earned = 1
            $contestDetails   = ContestDetails::where('contest_id', $activeContestId)->where('user_id', $freelancerReferrer)->get()->toArray();
            if(count($contestDetails)>0){
                foreach($contestDetails as $k => $contestDetail){
                    $pointsEarned = ($contestDetail["points_earned"]+1);
                }

                $contestDetails   = ContestDetails::where('contest_id', $activeContestId)->where('user_id', $freelancerReferrer)->first();
                $contestDetails->points_earned = $pointsEarned;
                $contestDetails->save();
            }
            else{
                $contestDetails   = new ContestDetails();
                $contestDetails->user_id = $freelancerReferrer;
                $contestDetails->contest_id = $activeContestId;
                $contestDetails->points_earned = 1;
                $contestDetails->save();
            }
        }

        if(!empty($jobPosterReferrer) && $jobPostersJobCount == $job_added_count){
            // add 1 contest point for the jobPosterReferrer if already exist in contest details table otherwise add new record with points_earned = 1

            $contestDetails   = ContestDetails::where('contest_id', $activeContestId)->where('user_id', $jobPosterReferrer)->get()->toArray();
            if(count($contestDetails)>0){
                foreach($contestDetails as $k => $contestDetail){
                    $pointsEarned = ($contestDetail["points_earned"]+1);
                }

                $contestDetails   = ContestDetails::where('contest_id', $activeContestId)->where('user_id', $jobPosterReferrer)->first();
                $contestDetails->points_earned = $pointsEarned;
                $contestDetails->save();
            }
            else{
                $contestDetails   = new ContestDetails();
                $contestDetails->user_id = $jobPosterReferrer;
                $contestDetails->contest_id = $activeContestId;
                $contestDetails->points_earned = 1;
                $contestDetails->save();
            }
    ...
    public function details($id)
    {
        ...
        $job       = JobPost::where('id', $id)->where('job_block_status', 0)->where('user_id', auth()->id())->with('proves.user')->firstOrFail();
