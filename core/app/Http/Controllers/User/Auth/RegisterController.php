...
class RegisterController extends Controller
{
    ...
    public function register(Request $request)
    {
       ...
       event(new Registered($user = $this->create($request->all())));
        ...
        if ($request->hasCookie($refCookieName)) {

            $referralCode = Cookie::get($refCookieName);
            $httpReferrer = Cookie::get($refCookieHttpReferrerName);

            $exist     = Referral::where('referral_code', $referralCode)->first();

            //Check exist or not
            if ($exist) {
                $referrerID    = $exist->user_id;
                $user->createReferralAccount($referrerID, $httpReferrer);
            }
    
            // Optionally, remove the cookie after processing
            Cookie::queue(Cookie::forget($refCookieName));
            Cookie::queue(Cookie::forget($refCookieHttpReferrerName));
        }
    ...
    public function checkUser(Request $request)
    {
        ...
        if ($ip) {
            $exist['data'] = User::where('ip_address', $ip)->exists();
            $exist['type'] = 'ip_address';
        }
