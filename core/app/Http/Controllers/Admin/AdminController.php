...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $job['total_job']    = JobPost::count();
        $job['pending_job']  = JobPost::pending()->count();
        $job['complete_job'] = JobPost::completed()->count();
        $job['cancel_job']   = JobPost::rejected()->count();

        $job['total_job_amount']    = JobPost::sum('total');
        $job['pending_job_amount']  = JobPost::pending()->sum('total');
        $job['complete_job_amount'] = JobPost::completed()->sum('total');
        $job['cancel_job_amount']   = JobPost::rejected()->sum('total');
        ...
        return view('admin.dashboard', compact('pageTitle', 'widget', 'chart', 'deposit', 'withdrawals', 'report', 'depositsMonth', 'withdrawalMonth', 'months', 'trxReport', 'plusTrx', 'minusTrx', 'job'));
    }
