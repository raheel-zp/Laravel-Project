...
class ManageUsersController extends Controller
{
    ...
    public function blockedUsers()
    {
        ...
        $users     = $this->userData('blocked');
        return view('admin.users.list', compact('pageTitle', 'users'));
    }

    public function executedBlockedUsers()
    {
        $pageTitle = 'Blocked Users for executing tasks';
        $users     = $this->userData('executionBlocked');
        return view('admin.users.list', compact('pageTitle', 'users'));
    }
    ...
    public function detail($id)
    {
        ...
        return view('admin.users.detail', compact('pageTitle', 'user', 'totalDeposit', 'totalWithdrawals', 'totalTransaction', 'countries', 'job'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $user->ev = $request->ev ? Status::ENABLE : Status::DISABLE;
        $user->sv = $request->sv ? Status::ENABLE : Status::DISABLE;
    ...
    public function blockStatus(Request $request, $id)
    {
        ...
        if ($user->block_status == 0 || $user->block_status == "") {
            ...
            $user->block_status = Status::USER_BLOCK;
    ...
    public function blockExecutionStatus(Request $request, $id)
    {
        $user = User::findOrFail($id);

        if ($user->block_execution_status == 0 || $user->block_execution_status == "") {
            $request->validate([
                'block_execution_reason' => 'required|string|max:255',
            ]);
            $user->block_execution_status = 1;
            $user->block_execution_reason = $request->block_execution_reason;
            $notify[]         = ['success', 'User blocked successfully'];
        } else {
            $user->block_execution_status = 0;
            $user->block_execution_reason = null;
            $notify[]         = ['success', 'User unblocked successfully'];
        }
